...
const sleepHabitLabels: Record<RoommateInfo['sleepHabit'], string> = {
...
const sleepPatternLabels: Record<RoommateInfo['sleepPattern'], string> = {
  sleep_early: '일찍 자요 (22시 ~ 00시)',
  sleep_late: '늦게 자요 (00시 ~ 03시)',
  sleep_irregular: '자는 시간 불규칙해요',
};

const noiseLabels: Record<RoommateInfo['noise'], string> = {
  noise_ok: '소음 상관 없어요',
  noise_little: '짧은 통화 정도만 원해요',
  noise_quiet: '조용한걸 원해요',
};

type RmCardConditionProps = Pick<
  RoommateInfo,
  'smoking' | 'indoorEating' | 'sleepHabit' | 'sleepPattern' | 'noise'
>;
