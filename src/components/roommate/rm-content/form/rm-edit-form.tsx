...
const RmEditForm = () => {
  const [profiles, setProfiles] = useState<RoomMateProfileType[]>([]);
  ...
  const handleUpdate = async (profile: RoomMateProfileType) => {
  ...
  const handleDelete = async (profile: RoomMateProfileType) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 px-2">
      {profiles.map((profile, index) => (
        <Card key={profile.id}>
          <RmContent
            label={`${profile.created_at?.split('T')[0]}에 등록한 프로필`}
            profile={profile}
            showMessageField
